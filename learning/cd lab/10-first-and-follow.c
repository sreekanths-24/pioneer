...
void findter()
{
 ...
 else if (pro[k].rhs[t][0] >= 65 && pro[k].rhs[t][0] 
<= 90)
 {
 temp = pro[k].rhs[t][0];
 if (temp == 'S')
...
ptr1 = strchr(pro[k].rhs[t], temp);
 ...
 {
 p1 = ptr1 - pro[k].rhs[t];
 if (pro[k].rhs[t][p1 + 1] >= 65 && pro[k].rhs[t][p1 
+ 1] <= 90)
 {
 for (o1 = 0; o1 < n; o1++)
...
pro[i].ft[strlen(pro[i].ft)] = pro[i].rhs[j][0];
 ...
 else if (pro[i].rhs[j][0] >= 65 && pro[i].rhs[j][0] <= 
90)
 {
 temp = pro[i].rhs[j][0];
 if (temp == 'S')
...
printf("\n\nFIRST\n");
 ...
 {
 p = ptr - pro[l].rhs[j];
 if (pro[l].rhs[j][p + 1] >= 65 && pro[l].rhs[j][p + 
1] <= 90)
 {
 for (o = 0; o < n; o++)
...
continue;
 ...
 pro[i].fol[strlen(pro[i].fol)] = pro[l].rhs[j][p + 
1];
 } } } }
 printf("\n\nFOLLOW\n");
 for (i = 0; i < n; i++)
